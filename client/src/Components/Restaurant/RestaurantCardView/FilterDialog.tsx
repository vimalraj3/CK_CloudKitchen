...
const SortBy: React.FC<IHandleSelectedFilters> = memo(
        ...
        <FormControl>
          <RadioGroup
            ...
            onChange={(event, value) =>
              handleSelectedFilters(filters.sortBy, value)
            }
          >
            <FormControlLabel
              value={sortBy.Rating}
...
const Animation: React.FC<IAnimation> = memo(({ children, selectedTab }) => {
  return (
...
const CheckBox: React.FC<ICheckbox> = memo(
  ({ handleSelectedFilters, setSelectedCuisines, selectedCuisines }) => {
    ...
    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (con == -1) {
        ...
        let update = selectedCuisines.filter(
          (value) => value != event.target.value
        )
        setSelectedCuisines(update)
        if (update.length === 0) handleSelectedFilters(filters.cuisines, `   `)
        else
          handleSelectedFilters(
            filters.cuisines,
...
const Cuisines: React.FC<IHandleSelectedFilters> = memo(
  ({ handleSelectedFilters }) => {
...
const Rating: React.FC<IHandleSelectedFilters> = memo(
  ({ handleSelectedFilters }) => {
    return (
      <div className="w-[80%] h-[250px]">
        <div className="flex h-[100%] w-[100%] mx-auto items-center">
          <Slider
            ...
            min={3}
            onChange={(event, value) =>
              handleSelectedFilters(filters.rating, `${value} +`)
            }
          />
        </div>
      </div>
...
const CostPerPerson = memo(
  ({ handleChange, value, handleSelectedFilters }: ICostPerPerson) => {
    return (
      <div className="h-[250px] flex items-center w-[80%] mx-auto ">
        <Slider
          ...
          onChange={(e, v) => {
            if (Array.isArray(v)) {
              handleSelectedFilters(
                filters.costPerPerson,
                `${v[0]} to ${v[1]} per person`
              )
              handleChange(e, v)
            }
          }}
...
const ActiveComponent: React.FC<IActiveComponent> = memo(
  ({ name, handleChange, value, handleSelectedFilters }) => {
    switch (name) {
      case filters.sortBy:
...
const Tab: React.FC<ITab> = memo(
  ({ filterOptions, setSelectedTab, selectedTab }) => {
    return (
        ...
        {filterOptions.map(({ name, selected }, i) => {
          return (
            <div className="flex flex-col basis-[25%]" key={i}>
              <p
                className={`px-2 w-[100 %] font-moutserrat text-[1.1rem] font-[300] pt-3 transition-all duration-600 ease-in-linear cursor-pointer flex flex-col items-center relative justify-center ${
                  name === selectedTab
                    ? 'bg-[#fff]  rounded-t-md '
              ...
              <p
                className={`px-2 w-[100%] font-moutserrat text-[.8rem] font-[300] pb-3 h-[24px] transition-all duration-600 ease-in-linear cursor-pointer flex  justify-center text-[#e03546] ${
                  name === selectedTab
                    ? 'bg-[#fff]  rounded-t-md '
                    : 'bg-[#f8f8f8]  rounded-md  '
                } `}
              ...
              {name === selectedTab ? (
                <motion.div
                  className="border-b-2 w-[100%] rounded-b-md mx-auto  border-[#ff7e8b]"
                  layoutId="underline"
                />
              ) : null}
            </div>
...
const FilterDialog: React.FC<IFilterDialog> = ({ open, setOpen, setBg }) => {
  ...
  const handleSelectedFilters = useCallback(
    (name: filters, selectedOption: string) => {
    ...
    >
      <div className="w-[90%]   max-w-[600px]  aspect-[4/3]  bg-[#fff] z-50 rounded-md flex flex-col justify-between">
        <section className="flex justify-between py-4 px-8 items-center border-b-2 border-[#c1c1c1] h-[15%]">
          ...
          <i
            className="fa-solid fa-xmark text-[1.4rem] cursor-pointer"
            onClick={toggleOpen}
          ></i>
        </section>
