...
function OfferCard() {
    return (
        <section className='w-[90%] mx-auto max-w-[1200px] mt-5 md:mt-10 flex justify-evenly items-center flex-col md:flex-row'>
            {
                offerCardData.map(({ title, offer, image, bgc }: IOfferCard, i: number) => {
                    return (
                        <div key={i} className={`w-[100%] md:w-[45%] max-w-[600px] my-5 px-2 py-3 md:py-4 md:px-8 aspect-[4/3] md:aspect-[16/9] rounded-lg flex flex-col-reverse  md:flex-row justify-between items-center`} style={{
                            backgroundColor: bgc
                        }}>
