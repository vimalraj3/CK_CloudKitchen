...
const userEditFormSchema = yup.object().shape({
    ...
    zipCode: yup.string().required("Zip code is required").matches(/^\d{6}$/, 'Invalid zip code')
})

export const UserAddressEditForm: React.FC<IUserAddressEditForm> = ({ address, handleSubmit }) => {
    return (
        <Form<UserAddressFormFields> defaultValues={address?.addressName ? address : defaultAddress} onSubmit={handleSubmit} schema={userEditFormSchema}>
            {({ register, errors }) => {
                return (
                    <>
                        <div>
                            <Input aria-label={'address name'} label="Address name"  {...register('addressName')} />
                            {errors.addressName && (
                                <Text
                        ...
                        <div>
                            <Input aria-label={'address name'} label="House no"   {...register('houseNo')} />
                            {errors.houseNo && (
                                <Text
                        ...
                        <div>
                            <Input aria-label={'street name'} label="Street name"   {...register('streetName')} />
                            {errors.streetName && (
                                <Text
                        ...
                        <div>
                            <Input aria-label={'state'} label="state"  {...register('state')} />
                            {errors.state && (
                                <Text
                        ...
                        <div>
                            <Input aria-label={'Zip code'} label="Zip code"  {...register('zipCode')} />
                            {errors.zipCode && (
                                <Text
